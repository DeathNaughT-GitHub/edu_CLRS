...
class _maximumSubarray {
  ...
  public static void main(String args[]) {
    ...
    System.out.println(
      tup.sum + " running from index: " + tup.x + " to " + tup.y
    );
  ...
  private static Tuple findMaxSub(int arr[], int start, int end) {
    ...
    if (M.sum >= L.sum && M.sum >= R.sum) return new Tuple(
      M.x,
      M.y,
      M.sum
    ); else if (L.sum >= M.sum && L.sum >= R.sum) return new Tuple(
      L.x,
      L.y,
      L.sum
    ); else return new Tuple(R.x, R.y, R.sum);
  }

  private static Tuple Max_Crossing_Sub(
    int arr[],
    int start,
    int mid,
    int end
